...
const supabase = createClient(
    "https://qcmdujvhpsexghhiwhah.supabase.co",
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InFjbWR1anZocHNleGdoaGl3aGFoIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MjgwMjA3NzYsImV4cCI6MjA0MzU5Njc3Nn0.AIDP79S8SC8Di_b91gtSusqBvVBlCVxBofSyucQkXUw"
);
...
const uploadDataToSupabase = async (row) => {
    //uploads the data as it is if there are no duplicates
    //if not it will update the data entry in the supabase
    let [new_category, new_indicator, new_indicator_name, new_remarks, new_units, new_year, new_value, new_company, std] = row;
    const { data, error } = await supabase
        .from('test') //change to esg_data for deployment
        .upsert([{
...
const uploadCompanyToSupabase = async (company) => {
...
const readCSV = async (filePath) => {
    ...
    return new Promise((resolve, reject) => {
        fs.createReadStream(filePath)
            .pipe(csv())  // csv-parser will parse each row into an object
            .on('data', (row) => {
                // Destructure the row object into individual variables
                const [index, category, indicator, indicator_name, unit, sub_category, year, value, company_name] = Object.values(row);

                // Push the processed row (as an array) to the results array
                results.push([index, category, indicator, indicator_name, unit, sub_category, year, value, company_name]);
            })
            .on('end', () => {
                resolve(results);  // Resolve with all processed rows
            })
            .on('error', (err) => {
...
const submitData = async (file, company_name) => {
    uploadCompanyToSupabase(company_name.trim())
    const csvData = await readCSV(file, company_name);
    try {
        ...
        for (let row of csvData) {
            let [index, category, indicator, indicator_name, unit, remarks, year, value, dummy] = row;  // Extract the last 6 elements (similar to *_, indicator, indicator_name, year, value, unit, remarks)
            remarks = remarks.trim()
            indicator_name = indicator_name.trim()
            indicator = indicator.trim()
            unit = unit.trim()
            year = year.trim()
            // Trim and convert remarks to uppercase
            // switch (remarks.toUpperCase()) {
            //     case 'PERCENTAGE OF TAILING WASTE RECYLCED':
            //         remarks = 'Percentage of tailing waste recycled'.toUpperCase();
            //     case 'PERCENTAGE OF MINERAL WASTE RECYLCED':
            //         remarks = 'Percentage of mineral waste recycled'.toUpperCase();
            //     case 'TOTAL NUMBER OF MHSA INCIDENTS':
            //         remarks = 'Total number of MSHA incidents'.toUpperCase();
            //     case 'AVERAGE HOURS OF HEALTH, SAFETY AND EMERGENCY RESPONSE TRAINING FOR FULL TIME EMPLOYEES':
            //         remarks = 'Average hours of health, safety, and emergency response training for full-time employees'.toUpperCase();
            //     default:
            ...
            if(['', ' ', null].includes(value)){
                ...
                value = parseInt(value.replace(/,/g, "")) //in case of commas
        ...
        for (let row of [...data]){
            ...
            let std2 = (parseFloat(value) - minMax_lst[0]) / (minMax_lst[1] - minMax_lst[0])
            if(std2){
            ...
            if(indicator_lst.includes([indicator.toUpperCase(), remarks.toUpperCase()].toString())){
                ...
                let std2_mp = (parseFloat(std1_mp) - minMax_lst_mp[0]) / (minMax_lst_mp[1] - minMax_lst_mp[0])
                let std2_rev = (parseFloat(std1_rev) - minMax_lst_rev[0]) / (minMax_lst_rev[1] - minMax_lst_rev[0])
                if(std1_mp){
...
const lst = ["IAMGOLD", "Pan American Silver", "Vale", "Anglo American", "Antofagasta (mining division)", "Kinross Gold", "Rio Tinto", "BHP", "Agnico Eagle",
    "Freeport McMoRan", "AngloGold Ashanti", "Mosaic", "South32", "Cameco", "Champion Iron", "Northern Star", "Norilsk Nickels", "Albemarle", "Nutrien", "Fortuna Silver Mines" ];

const lst2 = lst
for(let i = 0; i < lst2.length; i++){
    const fname = `C:/Users/tmxfe/OneDrive/Desktop/Acad/BT4103/SUBMIT MODULE/long_data/${lst2[i]}.csv`
    submitData(fname, lst2[i])
